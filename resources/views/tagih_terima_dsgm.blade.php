...
@section('title')
<title>TERIMA DOKUMEN SET - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Terima Dokumen Set</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url('/homepage') }}">Home</a></li>
            <li class="breadcrumb-item">Admin Trunojoyo</li>
            <li class="breadcrumb-item">Terima Dokumen Set</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
      ...
      <div class="card">
        <div class="card-header">
          <ul class="nav nav-tabs nav-tabs-lihat" id="custom-content-below-tab" role="tablist">
            <li class="nav-item">
              <a class="nav-link active" id="custom-content-below-home-tab" data-toggle="pill" href="#terima_data" role="tab" aria-controls="custom-content-below-home" aria-selected="true">Terima Dokumen</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" id="custom-content-below-profile-tab" data-toggle="pill" href="#dokumen_list" role="tab" aria-controls="custom-content-below-profile" aria-selected="false">Dokumen List</a>
            </li>
          </ul>
        </div>
        <div class="card-body">
          <div class="tab-content" id="custom-content-below-tabContent">
            ...
            <div class="tab-pane fade" id="dokumen_list" role="tabpanel" aria-labelledby="custom-content-below-messages-tab">
              <table id="data_dokumen_list_table" style="width: 100%;" class="table table-bordered table-hover responsive">
                <thead>
                  <tr>
                    <th>No</th>
                    <th>Tanggal</th>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#data_terima_table').DataTable({
         ...
         ajax: {
          url:'{{ url("admin/trunojoyo/terima_dsgm/table") }}',
    ...
    $('.nav-tabs a').on('shown.bs.tab', function (e) {
      ...
      if(target == '#terima_data'){
        ...
        load_data_terima();
      }else if(target == '#dokumen_list'){
        $('#data_dokumen_list_table').DataTable().destroy();
        load_data_dokumen_list();
      }
    });
    ...
    function load_data_dokumen_list(from_date = '', to_date = '')
    {
      table = $('#data_dokumen_list_table').DataTable({
        processing: true,
        serverSide: true,
        lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
        ajax: {
          url:'{{ url("admin/trunojoyo/dokumen_list/table") }}',
          data:{from_date:from_date, to_date:to_date},
          error: function(jqXHR, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + jqXHR.statusText + "\r\n" + jqXHR.responseText + "\r\n" + ajaxOptions.responseText);
          }
        ...
        {
          data:'tanggal_terima_trunojoyo',
          name:'tanggal_terima_trunojoyo',
          className:'dt-center',
          render: function ( data, type, full, meta ) {
            return moment(data).format('DD MMM YYYY');
          }
    ...
    function load_data_view(tanggal = '')
    {
      table = $('#detail_view_data_table').DataTable({
        ...
        lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
        ajax: {
          url:'{{ url("admin/trunojoyo/dokumen_list/view/table") }}',
    ...
    function load_data_terima_detail(tanggal = '')
    {
      table = $('#detail_data_terima_table').DataTable({
        ...
        bInfo : false,
        ajax: {
          url:'{{ url("admin/trunojoyo/terima_dsgm/detail/table") }}',
          data:{tanggal:tanggal},
          error: function(jqXHR, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + jqXHR.statusText + "\r\n" + jqXHR.responseText + "\r\n" + ajaxOptions.responseText);
          }
        ...
        {
          data:'check_diterima_trunojoyo',
          name:'check_diterima_trunojoyo',
          className:'dt-center',
          width:'7%',
          render: function ( data, type, row)
          {
    ...
    $('#filter').click(function(){
      ...
      if(from_date != '' &&  to_date != '')
      {
        if(target == '#terima_data'){
          ...
          $('#data_dokumen_list_table').DataTable().destroy();
          load_data_dokumen_list(from_date, to_date);
        }
      }
      else
      {
    ...
    $('body').on('click', '#receive-data', function () {
      ...
      document.getElementById("judul_modal_receive_data").innerHTML = "Data Tanggal " + moment(tanggal).format('DD MMM YYYY');
    ...
    $('#btn-save-data').click( function() {
      ...
      $.ajax({
        ...
        type: "POST",
        url: '{{ url("admin/trunojoyo/update/terima") }}',
        dataSrc : 'data',
        dataType: 'JSON',
        data: data,
        async: 'false',
