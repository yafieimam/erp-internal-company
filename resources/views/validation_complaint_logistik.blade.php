...
@section('title')
<title>VALIDASI COMPLAINT LOGISTIK - PT. DWI SELO GIRI MAS</title>
@endsection
...
@section('content')
<div class="container">
  <div class="title-view"><h2>Validasi Complaint Logistik</h2></div>
...
<script type="text/javascript">
    var baseurl = "{{ url('/validation_complaint_logistik') }}";
    ...
    $(document).ready(function(){
     ...
     function load_data(from_date = '', to_date = '')
     {
      $('#complaint_table').DataTable({
       ...
       serverSide: true,
       ajax: {
        url:'{{ route("complaintlogval.index") }}',
    ...
    $(document).ready(function(){
      $('body').on('click', '#validate-complaint', function () {
        ...
        if(confirm("Data Ini Akan Diubah Status Menjadi Done?")){
          $.ajax({
              type: "GET",
              url: "{{ url('complaint_logistik/validasi/') }}",
