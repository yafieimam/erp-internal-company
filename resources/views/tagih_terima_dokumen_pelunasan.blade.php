...
@section('title')
<title>TERIMA DOKUMEN PELUNASAN - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Terima Dokumen Pelunasan</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url('/homepage') }}">Home</a></li>
            <li class="breadcrumb-item">Penagihan</li>
            <li class="breadcrumb-item">Terima Dokumen Pelunasan</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div>
            <table id="detail_data_terima_table" style="width: 100%;" class="table table-bordered table-hover responsive">
              <thead>
                <tr>
                  <th>Nomor SJ</th>
                  <th>Customer</th>
...
<script type="text/javascript">
  $(document).ready(function () {
    var table = $('#data_terima_table').DataTable({
         ...
         ajax: {
          url:'{{ url("penagihan/terima/dokumen_pelunasan/table") }}',
    ...
    function load_data_terima_detail(tanggal = '')
    {
      table = $('#detail_data_terima_table').DataTable({
        ...
        bInfo : false,
        ajax: {
          url:'{{ url("penagihan/terima/dokumen_pelunasan/detail/table") }}',
        ...
        },
        {
          ...
          {
            if(data){
              return '<input type="checkbox" name="dikirim[' + $('<div />').text(row.nosj).html() + ']" value="1" checked="checked" disabled>';
            }else{
              return '<input type="checkbox" name="dikirim[' + $('<div />').text(row.nosj).html() + ']" value="1" disabled>';
            }
          }
        },
        {
          data:'check_diterima_dok_pelunasan',
          name:'check_diterima_dok_pelunasan',
          className:'dt-center',
          render: function ( data, type, row)
          {
            if(data){
    ...
    $('#btn-save-data').click( function() {
      ...
      $.ajax({
        ...
        type: "POST",
        url: '{{ url("penagihan/update/terima/dokumen_pelunasan") }}',
        dataSrc : 'data',
        dataType: 'JSON',
        data: data,
        async: 'false',
