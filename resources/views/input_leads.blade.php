...
@section('title')
<title>LEADS - PT. DWI SELO GIRI MAS</title>
@endsection
...
<style type="text/css">
  #leads_list_table tbody tr:hover{
    cursor: pointer;
  }
  .modal {
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Data Leads</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
  ...
  <div class="row"> 
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <table id="leads_list_table" style="width: 100%;" class="table table-bordered table-hover responsive">
            <thead>
              <tr>
                <th></th>
                <th>No</th>
    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Input Data Leads</h4>
  ...
  <div class="modal fade" id="modal_upload_excel">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form method="post" class="upload-form" id="upload-form" action="{{ url('/leads/upload_excel') }}" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="modal-body">
            <div class="form-group">
              <div class="custom-file">
            ...
            <span style="font-weight: 700;">Download file excel template <a href="{{asset('template/excel/template_leads.xlsx')}}" target="_blank">here</a>.</span>
          </div>
          <div class="modal-footer justify-content-between">
            <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary">Save changes</button>
  ...
  <div class="modal fade" id="modal_pembagian_sales">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Pembagian Sales Leads</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form method="post" class="edit_data_form" id="edit_data_form" action="javascript:void(0)" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input class="form-control" type="hidden" name="leadid_edit" id="leadid_edit" />
            <div class="row">
              <div class="col-4">
                <div class="form-group">
...
<script>
    ...
    $(document).ready(function(){
      ...
      var table = $('#leads_list_table').DataTable({
           processing: true,
           serverSide: true,
           responsive: {
            details: {
      ...
      if(any_nomor != ''){
        load_data_list_leads();
        table.ajax.url('{{ url("leads/specific/view/table") }}').load();
      }
      ...
      table = $('#pembagian_sales_table').DataTable({
       ...
       columns: [
        ...
        },
        {
         ...
         {
            return '<input type="hidden" name="kode_company[' + $('<div />').text(row.leadid).html() + ']" value="' + $('<div />').text(row.kode_company).html() + '">' + $('<div />').text(row.company).html();
         }
        },
        {
         data:'city',
        ...
        },
        {
         ...
         render: function ( data, type, row)
         {
            return '<input type="checkbox" name="pilih_sales[' + $('<div />').text(row.leadid).html() + ']" value="1"><input type="hidden" name="leadid[' + $('<div />').text(row.leadid).html() + ']" value="' + $('<div />').text(row.leadid).html() + '">';
         }
        },
        {
         data:null,
         ...
         render: function ( data, type, row)
         {
            return '<input type="checkbox" name="offline[' + $('<div />').text(row.leadid).html() + ']" value="1">';
         }
        },
        {
         data:null,
         ...
         render: function ( data, type, row)
         {
            return '<select name="perihal[' + $('<div />').text(row.leadid).html() + ']" style="width:100%;">' +
        ...
        },
        {
         ...
         render: function ( data, type, row)
         {
            $('[name="tanggal_kunjungan[' + $('<div />').text(row.leadid).html() + ']"]').flatpickr({
              allowInput: true,
              disableMobile: true
            });

            return '<input type="text" name="tanggal_kunjungan[' + $('<div />').text(row.leadid).html() + ']" style="width:100%; margin-bottom: 10px;" placeholder="Tanggal Kunjungan">';
         }
        }
       ]
      });
     ...
     function load_data_list_leads(from_date = '', to_date = '')
     {
      if(any_nomor != ''){
        table = $('#leads_list_table').DataTable({
           processing: true,
           serverSide: true,
           responsive: {
            details: {
    ...
    $('#leads_list_table').on( 'click', 'tbody tr', function () {
      var cust = table.row(this).data();
      var leadid = cust['leadid'];
      document.getElementById("detail_judul").innerHTML = "Detail Leads : " + leadid;
      var url = "{{ url('leads/view/leadid') }}";
      url = url.replace('leadid', enc(leadid.toString()));
      $('#td_leadid').html('');
      $('#td_nama').html('');
      $('#td_crd').html('');
      $('#td_pic').html('');
      ...
      $.get(url, function (data) {
        ...
        $('#btn-delete-cust').data('id', data.leadid);
        $('#btn-edit-cust').data('id', data.leadid);
        $('#btn-history-cust').data('id', data.leadid);
      })
    });
    ...
    $('#btn-edit-cust').click(function (e) {
      var lead_id_edit = $(this).data("id");
      document.getElementById("modal-edit-title").innerHTML = "Edit Leads " + lead_id_edit;
      ...
      var url = "{{ url('leads/view/leadid') }}";
      url = url.replace('leadid', enc(lead_id_edit.toString()));
      $('#leadid_edit').val('');
      $('#name_edit').val('');
      $('#address_edit').html('');
      $('#crd_edit').html('');
    ...
    $('#filter').click(function(){
      ...
      {
        $('#leads_list_table').DataTable().destroy();
        load_data_list_leads(from_date, to_date);
      }
      else
      {
  ...
  $(document).ready(function(){
    $('body').on('click', '#btn-delete-cust', function () {
      ...
      if(confirm("Data Dihapus?")){
        $.ajax({
          type: "GET",
          url: "{{ url('leads/reject') }}",
          data: { 'leadid' : leadid },
          success: function (data) {
            var oTable = $('#leads_list_table').dataTable(); 
            oTable.fnDraw(false);
        ...
        $.ajax({
          ...
          success:function(data){
            if(data.status == false){
              ...
              var oTable = $('#data_belum_lengkap_table').dataTable();
              oTable.fnDraw(false);
              $('#modal_input_data').modal('hide');
              $("#modal_input_data").trigger('click');
              alert("Leads Data Successfully Added");
            }
          },
          error: function (data) {
            console.log('Error:', data);
        ...
        $.ajax({
          ...
          success:function(data){
            if(data.status == false){
              ...
              alert("Leads Data Successfully Updated");
            }
          },
          error: function (data) {
            console.log('Error:', data);
