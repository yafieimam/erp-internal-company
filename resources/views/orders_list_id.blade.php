...
@section('title')
<title>LIST PESANAN - PT. DWI SELO GIRI MAS</title>
@endsection
...
@section('nav')
            <div class="bahasa">
                <p>
                  <a href="{{ url('en/orders/list') }}"><span class="">EN</span></a>
                  &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
                  <a href="{{ url('id/orders/list') }}"><span class="active">IN</span></a>
                </p>
            </div>
@endsection
...
<section>
  <div class="container">
    <span class="login100-form-title wow fadeInDown">List Pesanan</span>
    <div class="row">
      <div class="col-sm-3">
              ...
              <div class="panel-heading">
                <h4 class="panel-title">
                  <ul class="nav nav-tabs" style="border-bottom: 0;">
                    ...
                    <li><a href="#new" data-toggle="tab">Pesanan Baru</a></li>
                    <li><a href="#confirm" data-toggle="tab">Konfirmasi</a></li>
                    <li><a href="#produksi" data-toggle="tab">Produksi</a></li>
                    <li><a href="#delivery" data-toggle="tab">Pengiriman</a></li>
                    <li><a href="#transit" data-toggle="tab">Sedang Transit</a></li>
                    <li><a href="#arrive" data-toggle="tab">Pesanan Tiba</a></li>
                  </ul>
                </h4>
              </div>
            </div>
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-sm-12 padding-right">
                    ...
                    <div class="product-information"><!--/product-information-->
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                          ...
                          <input class="form-control" type="text" name="nomor_po" id="nomor_po" placeholder="Nomor PO" readonly/>
                        </div>
                      </div>
                  ...
                  <div class="view-product">
                    <div class="product-information"><!--/product-information-->
                      <div class="col-sm-12" id="div-keterangan">
                        <div class="form-group">
                          <label for="keterangan">Informasi Lainnya</label>
              ...
              <div class="product-details" id="btn-confirm-orderan" style="display: none;"><!--product-details-->
                <div class="col-sm-3"></div>
                <div class="col-sm-3">
                  <input class="btn btn-danger" type="button" name="btn-orders-cancel" id="btn-orders-cancel" value="Batal" style="margin-top: 5px; height: 40px;"/>
                </div>
                <div class="col-sm-3">
                  <input class="btn btn-success" type="button" name="btn-orders-confirm" id="btn-orders-confirm" value="Setuju" style="margin-top: 5px; height: 40px;"/>
                </div>
                <div class="col-sm-3"></div>
              </div>
            </div>
...
<script type="text/javascript">
    var baseurl = "{{ url('id/orders/list') }}";
    ...
    $('body').on('click', '#detail_order', function () {
      ...
      var nomor_sj = $(this).data('sj');
        ...
        $.get(url, function (data_prd) {
          ...
          $('#title-detail-order').html('Detail Pesanan ' + nomor_sj);
          $.get(url_data, function (data) {
            $('#nama_customer_order').html(data.nama_cust_order);
            $('#phone_customer_order').html('<b>Telepon: </b>' + data.phone_cust_order);
            $('#alamat_customer_order').html('<b>Alamat: </b>' + data.alamat_cust_order);
            $('#kota_customer_order').html('<b>Kota: </b>' + data.city_cust_order);
            $('#npwp_customer_order').html('<b>NPWP: </b>' + data.npwp_cust_order);
            if(data.crd == 1){
              $('#crd_customer_order').html('<b>Kredit Pembayaran: </b>' + data.crd + ' Hari Tunai / Transfer');
            }else{
              $('#crd_customer_order').html('<b>Kredit Pembayaran: </b>' + data.crd + ' Hari');
            }
            $('#nama_customer_receive').html(data.nama_cust_receive);
            $('#phone_customer_receive').html('<b>Telepon: </b>' + data.phone_cust_receive);
            $('#alamat_customer_receive').html('<b>Alamat: </b>' + data.alamat_cust_receive);
            $('#kota_customer_receive').html('<b>Kota: </b>' + data.city_cust_receive);
        ...
        $('body').on('click', '#btn-orders-confirm', function () {
          if(confirm("Setuju Dengan Detail Pesanan Diatas?")){
            $.ajax({
              type: "GET",
              url: "{{ url('validasi_orders') }}",
              data: { 'nomor_sj' : nomor_sj },
              success: function (data) {
                alert("Pesanan Berhasil Disetujui. Pesanan Anda Akan Diproduksi.");
                $("#modal_detail_order").modal('hide');
                $('body').removeClass('modal-open');
                $('.modal-backdrop').remove();
                $('#all').load('/list/id/all?page=1');
                $('#new').load('/list/id/new?page=1');
                $('#confirm').load('/list/id/confirm?page=1');
                $('#produksi').load('/list/id/produksi?page=1');
                $('#delivery').load('/list/id/delivery?page=1');
                $('#transit').load('/list/id/transit?page=1');
                $('#arrive').load('/list/id/arrive?page=1');
              },
              error: function (data) {
        ...
        $('body').on('click', '#btn-orders-cancel', function () {
          if(confirm("Anda Yakin Ingin Membatalkan Pesanan?")){
            $.ajax({
              type: "GET",
              url: "{{ url('cancel_orders') }}",
              data: { 'nomor_sj' : nomor_sj },
              success: function (data) {
                alert("Pesanan Anda Telah Dibatalkan. Terima Kasih Telah Memesan di PT Dwi Selo Giri Mas");
                $("#modal_detail_order").modal('hide');
              },
              error: function (data) {
                console.log('Error:', data);
      ...
      $('#filter').click(function(){
        ...
        if(from_date != '' &&  to_date != '')
        {
          ...
          // first load
          $('#all').load('/list/id/filter/all/' + from_date + '/' + to_date + '?page=1');
          $('#new').load('/list/id/filter/new/' + from_date + '/' + to_date + '?page=1');
          $('#confirm').load('/list/id/filter/confirm/' + from_date + '/' + to_date + '?page=1');
          $('#produksi').load('/list/id/filter/produksi/' + from_date + '/' + to_date + '?page=1');
          $('#delivery').load('/list/id/filter/delivery/' + from_date + '/' + to_date + '?page=1');
          $('#transit').load('/list/id/filter/transit/' + from_date + '/' + to_date + '?page=1');
          $('#arrive').load('/list/id/filter/arrive/' + from_date + '/' + to_date + '?page=1');
        }
        else
        {
