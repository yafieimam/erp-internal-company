...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#data_pembayaran_table').DataTable({
      ...
      ajax: {
        url:'{{ url("penagihan/pembayaran/table") }}',
    ...
    function load_data_list_payment_receive(from_date = '', to_date = '')
    {
      table = $('#data_list_payment_receive_table').DataTable({
        ...
        lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
        ajax: {
          url:'{{ url("penagihan/list_payment_receive/table") }}',
          data:{from_date:from_date, to_date:to_date},
          error: function(jqXHR, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + jqXHR.statusText + "\r\n" + jqXHR.responseText + "\r\n" + ajaxOptions.responseText);
          }
        ...
        },
        {
          ...
          {
              return $('<div />').text(row.nama_metode_pembayaran).html();
          }
        },
        {
          data:'nominal_bayar',
    ...
    function load_data_pembayaran_detail(tanggal = '')
    {
      table = $('#data_pembayaran_detail_table').DataTable({
        ...
        bInfo : false,
        ajax: {
          url:'{{ url("penagihan/pembayaran/detail/table") }}',
        ...
        },
        {
          ...
          render: function ( data, type, row)
          {
            ...
            return '<label>Rp : </label> <input type="text" name="nominal[' + $('<div />').text(row.nosj).html() + ']" style="width:80%; margin-top: 10px;" placeholder="Nominal">';
          }
        }
        ]
      });
    ...
    $('#btn-save-data').click( function() {
      ...
      $.ajax({
        ...
        type: "GET",
        url: '{{ url("penagihan/pembayaran/save") }}',
        dataSrc : 'data',
        dataType: 'JSON',
        data: data,
        async: 'false',
