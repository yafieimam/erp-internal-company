...
@section('title')
<title>IZIN - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Data Izin</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
  ...
  <div class="row"> 
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <table id="data_izin_table" style="width: 100%;" class="table table-bordered table-hover responsive">
            <thead>
              <tr>
                <th>No</th>
                <th>Tanggal</th>
        ...
        <div class="modal-body">
          <form method="post" class="input_form" id="input_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="nomor_surat_izin">Nomor Surat Izin</label>
                  <input type="text" class="form-control" name="nomor_surat_izin" id="nomor_surat_izin" autocomplete="off" placeholder="Nomor Surat Izin">
                </div>
              </div>
            </div>
            <div class="row">
              ...
              <div class="col-12">
                <div class="form-group">
                  ...
                  <select id="status" name="status" class="form-control" style="width: 100%;">
                    <option value="1" selected>Izin</option>
          ...
          <div class="row">
            <div class="col-lg-12">
              <table class="table" style="border: none;" id="lihat-table">
                <tr>
                  <td>Nomor Surat Izin</td>
                  <td>:</td>
                  <td id="td_nomor_surat_izin"></td>
                </tr>
                <tr>
                  <td>Karyawan</td>
                  <td>:</td>
        ...
        <div class="modal-body">
          <form method="post" class="edit_form" id="edit_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="edit_nomor_surat_izin">Nomor Surat Izin</label>
                  <input type="text" class="form-control" name="edit_nomor_surat_izin" id="edit_nomor_surat_izin" autocomplete="off" placeholder="Nomor Surat Izin">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  ...
                  <textarea class="form-control" rows="3" name="edit_alasan" id="edit_alasan" placeholder="Alasan"></textarea>
                </div>
              </div>
            </div>
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#data_izin_table').DataTable({
         processing: true,
         serverSide: true,
    ...
    function load_data_izin(from_date = '', to_date = '')
    {
      table = $('#data_izin_table').DataTable({
         processing: true,
         serverSide: true,
         lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
         ajax: {
    ...
    $('#filter').click(function(){
      ...
      {
        $('#data_izin_table').DataTable().destroy();
        load_data_izin(from_date, to_date);
      }
      else
      {
    ...
    $('body').on('click', '#view-data', function () {
      var id = $(this).data("id");
      var url = "{{ url('izin/view/id') }}";
      url = url.replace('id', enc(id.toString()));
      $('#td_nomor_surat_izin').html('');
      $('#td_tanggal').html('');
      $('#td_karyawan').html('');
      $('#td_status').html('');
      $('#td_alasan').html('');
      $.get(url, function (data) {
        $('#td_nomor_surat_izin').html(data.nomor_surat_izin);
        $('#td_tanggal').html(data.tanggal);
        $('#td_karyawan').html(data.karyawan);
        if(data.status == 1){
          $('#td_status').html("Izin");
        }else if(data.status == 2){
          $('#td_status').html("Sakit");
        }
        if(data.alasan == null || data.alasan == ''){
          $('#td_alasan').html("---");
        }else{
          $('#td_alasan').html(data.alasan);
        }
      })
    });
    ...
    $('body').on('click', '#edit-data', function () {
      ...
      url = url.replace('id', enc(id.toString()));
      $('#edit_id').val(id);
      $('#edit_nomor_surat_izin').val('');
      $('#edit_tanggal').val('');
      $('#edit_karyawan').val('').trigger('change');
      $('#edit_status').val('');
      $('#edit_alasan').html('');
      $.get(url, function (data) {
        $('#edit_nomor_surat_izin').val(data.nomor_surat_izin);
        $('#edit_tanggal').val(data.tanggal);
        $('#edit_status').val(data.status);
        $('#edit_alasan').html(data.alasan);
      ...
      if(confirm("Data Dihapus?")){
        $.ajax({
          ...
          success: function (data) {
            var oTable = $('#data_izin_table').dataTable();
            oTable.fnDraw(false);
            alert("Data Deleted");
          },
          error: function (data) {
    ...
    $('#input_form').validate({
      ...
      messages: {
        ...
        nomor_surat_izin: {
          required: "Nomor Surat Izin Harus Diisi",
        },
        karyawan: {
