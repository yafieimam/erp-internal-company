...
@section('title')
<title>LAPORAN TOTAL PRODUKSI - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Laporan Total Produksi</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url('/homepage') }}">Home</a></li>
            <li class="breadcrumb-item">Produksi</li>
            <li class="breadcrumb-item">Laporan Total Produksi</li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
      ...
      <div class="card">
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-2">
              <a href="{{ url('laporan_total_produksi/excel') }}" class="btn btn-primary" id="btn-save-excel" style="width: 100%;">Download Excel</a>
            </div>
          </div>
        </div>
        <div class="card-body">
          <div class="tab-content" id="custom-content-below-tabContent">
            ...
            <div class="tab-pane fade" id="grafik_perbandingan" role="tabpanel" aria-labelledby="custom-content-below-messages-tab">
              <div class="chart">
                <canvas id="chartTotalProduksiTotal" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiSA" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiSB" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiMixer" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRA" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRB" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRC" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRD" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRE" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRF" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
              <div class="chart">
                <canvas id="chartTotalProduksiRG" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
              </div>
            </div>
          </div>
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    {
      var url = "{{ url('laporan_total_produksi/view/data') }}";
      $("#tbody_view").empty();
            ...
            if(kt == 0){
              ...
              $.each(arr_bulan, function(kb, vb) {
                if(data[vm][vt][vb] ==  null){
                  ...
                  $('#tr_mesin_'+vm).append(
                    '<td style="vertical-align : middle; text-align: center;">'+numberWithCommas(data[vm][vt][vb])+'</td>'
                  );
                }
              });
            }else{
          ...
          if(kt == 0){
            ...
            $.each(arr_bulan, function(kb, vb) {
              if(total[vt][vb] ==  null){
                ...
                $('#tr_total').append(
                  '<td style="vertical-align : middle; text-align: center;">'+numberWithCommas(total[vt][vb])+'</td>'
                );
              }
            });
          }else{
    ...
    function load_grafik_perbandingan()
    {
      ...
      $.ajax({
        type: "GET",
        url: "{{ url('laporan_total_produksi/grafik/data') }}",
          ...
          $.each(arr_mesin, function(k, v) {
            ...
            for(var i in data[v]) {
              if(data[v][i].tahun == '{{ date("Y") }}'){
                total1.splice(data[v][i].bulan - 1, 0, data[v][i].tonase);
                total1.join();
              }else if(data[v][i].tahun == "{{ date('Y', strtotime('-1 year')) }}"){
                total2.splice(data[v][i].bulan - 1, 0, data[v][i].tonase);
                total2.join();
              }else if(data[v][i].tahun == "{{ date('Y', strtotime('-2 year')) }}"){
                total3.splice(data[v][i].bulan - 1, 0, data[v][i].tonase);
                total3.join();
              }
            }
            ...
            var barChartCanvas = $('#chartTotalProduksi'+v).get(0).getContext('2d');
            var barChartOptions = {
              responsive              : true,
