...
@section('title')
<title>CUTI - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Data Cuti</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
  ...
  <div class="row"> 
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <table id="data_cuti_table" style="width: 100%;" class="table table-bordered table-hover responsive">
            <thead>
              <tr>
                <th>No</th>
                <th>Tanggal</th>
        ...
        <div class="modal-body">
          <form method="post" class="input_form" id="input_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="nomor_surat_cuti">Nomor Surat Cuti</label>
                  <input type="text" class="form-control" name="nomor_surat_cuti" id="nomor_surat_cuti" autocomplete="off" placeholder="Nomor Surat Cuti">
                </div>
              </div>
            </div>
            <div class="row">
          ...
          <div class="row">
            <div class="col-lg-12">
              <table class="table" style="border: none;" id="lihat-table">
                <tr>
                  <td>Nomor Surat Cuti</td>
                  <td>:</td>
                  <td id="td_nomor_surat_cuti"></td>
                </tr>
                <tr>
                  <td>Karyawan</td>
                  <td>:</td>
        ...
        <div class="modal-body">
          <form method="post" class="edit_form" id="edit_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="edit_nomor_surat_cuti">Nomor Surat Cuti</label>
                  <input type="text" class="form-control" name="edit_nomor_surat_cuti" id="edit_nomor_surat_cuti" autocomplete="off" placeholder="Nomor Surat Cuti">
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#data_cuti_table').DataTable({
         processing: true,
         serverSide: true,
    ...
    function load_data_cuti(from_date = '', to_date = '')
    {
      table = $('#data_cuti_table').DataTable({
         processing: true,
         serverSide: true,
         lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
         ajax: {
    ...
    $('#filter').click(function(){
      ...
      {
        $('#data_cuti_table').DataTable().destroy();
        load_data_cuti(from_date, to_date);
      }
      else
      {
    ...
    $('body').on('click', '#view-data', function () {
      var id = $(this).data("id");
      var url = "{{ url('cuti/view/id') }}";
      url = url.replace('id', enc(id.toString()));
      $('#td_nomor_surat_cuti').html('');
      $('#td_tanggal').html('');
      $('#td_karyawan').html('');
      $('#td_keterangan').html('');
      $.get(url, function (data) {
        $('#td_nomor_surat_cuti').html(data.nomor_surat_cuti);
    ...
    $('body').on('click', '#edit-data', function () {
      ...
      url = url.replace('id', enc(id.toString()));
      $('#edit_id').val(id);
      $('#edit_nomor_surat_cuti').val('');
      $('#edit_tanggal').val('');
      $('#edit_karyawan').val('').trigger('change');
      $('#edit_keterangan').html('');
      $.get(url, function (data) {
        $('#edit_nomor_surat_cuti').val(data.nomor_surat_cuti);
      ...
      if(confirm("Data Dihapus?")){
        $.ajax({
          ...
          success: function (data) {
            var oTable = $('#data_cuti_table').dataTable();
            oTable.fnDraw(false);
            alert("Data Deleted");
          },
          error: function (data) {
    ...
    $('#input_form').validate({
      ...
      messages: {
        ...
        nomor_surat_cuti: {
          required: "Nomor Surat Cuti Harus Diisi",
        },
        karyawan: {
