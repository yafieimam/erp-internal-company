...
@section('title')
<title>HARI BESAR - PT. DWI SELO GIRI MAS</title>
@endsection
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Data Hari Besar</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
  ...
  <div class="row"> 
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <table id="data_hari_besar_table" style="width: 100%;" class="table table-bordered table-hover responsive">
            <thead>
              <tr>
                <th>No</th>
                <th>Tanggal</th>
        ...
        <div class="modal-body">
          <form method="post" class="input_form" id="input_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="hari_libur">Hari Libur</label>
                  <textarea class="form-control" rows="3" name="hari_libur" id="hari_libur" placeholder="Hari Libur"></textarea>
                </div>
              </div>
            </div>
        </div>
        ...
        <div class="modal-body">
          <form method="post" class="edit_form" id="edit_form" action="javascript:void(0)" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="edit_hari_libur">Hari Libur</label>
                  <textarea class="form-control" rows="3" name="edit_hari_libur" id="edit_hari_libur" placeholder="Hari Libur"></textarea>
                </div>
              </div>
            </div>
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#data_hari_besar_table').DataTable({
         processing: true,
         serverSide: true,
         lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
         ajax: {
          url:'{{ url("hari_besar/table") }}',
    ...
    function load_data_hari_besar(from_date = '', to_date = '')
    {
      table = $('#data_hari_besar_table').DataTable({
         processing: true,
         serverSide: true,
         lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
         ajax: {
    ...
    $('#filter').click(function(){
      ...
      {
        $('#data_hari_besar_table').DataTable().destroy();
        load_data_hari_besar(from_date, to_date);
      }
      else
      {
    ...
    $('#refresh').click(function(){
      $('#filter_tanggal').val('');
      $('#data_hari_besar_table').DataTable().destroy();
      load_data_hari_besar();
    });

    $('body').on('click', '#view-data', function () {
      var id = $(this).data("id");
      var url = "{{ url('hari_besar/view/id') }}";
      ...
      $.get(url, function (data) {
        ...
        $('#td_hari_libur').html(data.hari_libur);
      })
    });
    ...
    $('body').on('click', '#edit-data', function () {
      ...
      $('#edit_id').val('');
      $('#edit_tanggal').val('');
      $('#edit_hari_libur').html('');
      $.get(url, function (data) {
        $('#edit_id').val(data.id);
        $('#edit_tanggal').val(data.tanggal);
        $('#edit_hari_libur').html(data.hari_libur);
      })
    });
    ...
    $('body').on('click', '#delete-data', function () {
      ...
      if(confirm("Data Dihapus?")){
        $.ajax({
          type: "GET",
          url: "{{ url('hari_besar/delete') }}",
          data: { 'id' : id },
          success: function (data) {
            var oTable = $('#data_hari_besar_table').dataTable();
            oTable.fnDraw(false);
            alert("Data Deleted");
          },
          error: function (data) {
    ...
    $('#input_form').validate({
      ...
      messages: {
        ...
        hari_libur: {
          required: "Hari Libur Harus Diisi",
        },
      },
      errorElement: 'span',
