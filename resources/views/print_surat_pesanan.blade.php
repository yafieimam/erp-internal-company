    ...
    @foreach($data as $data)
            ...
            <tr>
                <td>
                    <tr>
                        ...
                        <td style="font-size: 12px; font-weight: 0; width: 25%; border-bottom: 1px solid #000;">{{ $data->nomor_order_receipt }}
                    ...
                    <tr>
                        ...
                        <td style="font-size: 12px; font-weight: 0; width: 20%; border-bottom: 1px solid #000;">{{ date('j F Y', strtotime($data->tanggal_order)) }}</td>
                    </tr>
                    <tr>
        ...
        <table style="font-size: 12px; font-weight: 0; width: 100%; text-align: center; border-collapse: collapse;">
            ...
            @foreach($products as $prod)
                @if($data->id_alamat == $prod->id_alamat)
                    {{ $sum+= $prod->total_price }}
                    {{ $sum_noppn += $prod->qty * $prod->harga_satuan }}
                    {{ $sum_diskon = $sum_noppn * $discount / 100 }}
                    {{ $sum_after_diskon = $sum_noppn - $sum_diskon }}
                    {{ $keterangan_quotation .= $prod->keterangan_quotation }}
                    <tr style="">
            ...
            <tr style="">
                <td style="width: 20%;border-bottom: 1px solid #000; padding: 3px; text-align: left;">Discount ({{ $discount }}%)</td>
                <td colspan="3" style="border-right: 1px solid #000; border-bottom: 1px solid #000; padding: 3px;"></td>
                <td style="width: 15%; border-bottom: 1px solid #000; padding: 3px;">{{ number_format($sum_diskon, 0, ".", ".") }}</td>
            </tr>
            <tr style="">
                <td style="width: 20%;border-bottom: 1px solid #000; padding: 3px; height: 12px; text-align: left;">PPN ({{ $ppn }}%)</td>
                <td colspan="3" style="border-right: 1px solid #000; border-bottom: 1px solid #000; padding: 3px;"></td>
                <td style="width: 15%; border-bottom: 1px solid #000; padding: 3px;">{{ number_format($sum_after_diskon * $ppn / 100, 0, ".", ".") }}</td>
            </tr>
            <tr style="">
        ...
        <table style="width: 100%;">
            ...
            <tr>
                ...
                <td style="font-size: 12px; font-weight: 0; border-bottom: 1px solid #000; width: 50%;">{{ $data->keterangan_order . ' ' . $keterangan_quotation }}</td>
            </tr>
        </table>
        <br>
