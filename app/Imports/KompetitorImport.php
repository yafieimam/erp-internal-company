...
class KompetitorImport implements ToModel, WithStartRow
{
    /**
    * @param array $row
    *
    ...
    */
    ...
    public function model(array $row)
    {
        ...
        $kode_cust = 'KOMP' . $data_provinsi->kode . $data_city->kode . $kode_nama;
        $data_kompid = ModelKompetitor::where('kompid', 'like', '%' . $kode_cust . '%')->orderBy('kompid', 'asc')->get();

        if($data_kompid){
        	$data_count = $data_kompid->count();
        	if($data_count > 0){
        		$num = (int) substr($data_kompid[$data_kompid->count() - 1]->kompid, 15);
        		if($data_count != $num){
        			$kode_cust = ++$data_kompid[$data_kompid->count() - 1]->kompid;
        		}else{
        			if($data_count < 9){
        ...
        if(!ModelKompetitor::where('kompid', '=', $kode_cust)->exists()) {
            $this->duplikat = 1;

            date_default_timezone_set('Asia/Jakarta');
        	DB::table('logbook_kompetitor')->insert(['tanggal' => date("Y-m-d H:i:s"), 'id' => $kode_cust, 'status' => 2, 'action' => 'Data Kompetitor ' . $kode_cust . ' Dari Upload Excel']);

            return new ModelKompetitor([
            	'kompid' => $kode_cust,
                'id_sales' => $sales_id,
