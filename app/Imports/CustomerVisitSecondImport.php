...
class CustomerVisitSecondImport implements ToModel, WithStartRow
{
    /**
        ...
        if($dsgm_yes){
        	...
        	if(empty($cust)){
        		...
        		if(empty($cust2)){
        			...
        			$data->custname = preg_replace('/_x([0-9a-fA-F]{4})_/', '', $row[5]);
        			$data->company = 'DSGM';
        			$data->city = $data_kota;
        	...
        	$tanggal_schedule = date('ym');
       		...
       		date_default_timezone_set('Asia/Jakarta');
       		$data = DB::table('customer_follow_up')->insert(["id_schedule" => $kode_schedule, "id_user" => Session::get('id_user_admin'), "customers" => $data_cust_dsgm, "perihal" => "Customer Visit", "offline" => 0, "keterangan" => $row[10], "penawaran_yes" => $penawaran_yes, "sample_yes" => $sample_yes, "order_yes" => $order_yes, "catatan_penawaran" => $catatan_penawaran, "catatan_sample" => $catatan_sample, "catatan_order" => $catatan_order, "tanggal_schedule" => $this->transformDate($row[1]), "status" => 1, "created_by" => Session::get('id_user_admin'), "created_at" => date("Y-m-d H:i:s"), "updated_by" => Session::get('id_user_admin'), "updated_at" => date("Y-m-d H:i:s")]);
