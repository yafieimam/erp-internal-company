...
class ComplaintTimbangController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($request->hasFile('upload_file_timbang')) {
            $file = $request->file('upload_file_timbang');
            $nama_file = time()."_".$request->custid_timbang."_".$request->nomor_complaint_timbang."_".$file->getClientOriginalName();
            $tujuan_upload = 'data_file';
            $file->move($tujuan_upload, $nama_file);

            $data_komitmen = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_timbang)->where('divisi', 4)->first();

            if($data_komitmen){
                $data_komitmen_get = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_timbang)->where('divisi', 4)->get();
            ...
            $complaint=DB::table('complaint_timbangan')->insert(["nomor_complaint" => $request->nomor_complaint_timbang, "evaluasi" => $request->evaluasi_timbang, "solusi_internal" => $request->solusi_internal_timbang, "solusi_customer" => $request->solusi_customer_timbang, "lampiran" => $nama_file, "created_by" => Session::get('id_user_admin'), "tanggal_input" => date('Y-m-d')]);

            if($complaint){
                $arr = array('msg' => 'Data Successfully Stored', 'status' => true);
                DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_timbang)->update(['baca' => 0, 'timbangan' => 2]);

                $stat = DB::table('status_complaint')->select(DB::raw("(if(produksi = 2, 2, 0) + if(logistik = 2, 2, 0) + if(sales = 2, 2, 0) + if(timbangan = 2, 2, 0) + if(warehouse = 2, 2, 0) + if(lab = 2, 2, 0) + if(lainnya = 2, 2, 0)) as hasil"))->where('nomor_complaint', $request->nomor_complaint_timbang)->first();

                $div = DB::table('divisi_complaint')->select(DB::raw("(if(produksi = 2, 2, 0) + if(logistik = 2, 2, 0) + if(sales = 2, 2, 0) + if(timbangan = 2, 2, 0) + if(warehouse = 2, 2, 0) + if(lab = 2, 2, 0) + if(lainnya = 2, 2, 0)) as hasil"))->where('nomor_complaint', $request->nomor_complaint_timbang)->first();

                if($stat->hasil == $div->hasil){
                    DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_timbang)->update(['validasi' => 1]);
                }else{
                    DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_timbang)->update(['validasi' => 0]);
                }
            }
        }else{
            $data_komitmen = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_timbang)->where('divisi', 4)->first();
            ...
            $complaint=DB::table('complaint_timbangan')->insert(["nomor_complaint" => $request->nomor_complaint_timbang, "evaluasi" => $request->evaluasi_timbang, "solusi_internal" => $request->solusi_internal_timbang, "solusi_customer" => $request->solusi_customer_timbang, "created_by" => Session::get('id_user_admin'), "tanggal_input" => date('Y-m-d')]);
        ...
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $request->nomor_complaint_timbang, 'divisi' => 4, 'action' => 'Divisi Timbangan Berhasil Memproses dan Memasukkan Data pada Complaint Nomor ' . $request->nomor_complaint_timbang]);
        ModelComplaintCust::where('nomor_complaint', $request->nomor_complaint_timbang)->update(['updated_at' => date("Y-m-d H:i:s")]);
    ...
    public function show($nomor_complaint){   
        ...
        $data  = DB::table('complaint_timbangan as com')->select('com.nomor_complaint', 'com.evaluasi', 'com.solusi_internal', 'com.solusi_customer', 'com.lampiran')->where('com.nomor_complaint', $val_nomor_complaint)->first();
     
        $komitmen = DB::table('list_of_action_complaint')->select('komitmen', 'selesai_tanggal_komitmen')->where('nomor_complaint', $val_nomor_complaint)->where('divisi', 4)->get();
     
        return Response()->json(["data_complaint" => $data, "data_komitmen" => $komitmen]);
    }

    public function showDataTimbangan($no_complaint){ 
    ...
    public function complaintTdkTerlibat(Request $request){
        ...
        DB::table('divisi_complaint')->where('nomor_complaint', $no_complaint)->update(['timbangan' => 1, 'alasan_timbangan' => $request->alasan_tidak_terlibat]);
        ...
        date_default_timezone_set('Asia/Jakarta');
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $no_complaint, 'divisi' => 4, 'action' => 'Divisi Timbangan Merubah Status Menjadi Tidak Terlibat Pada Complaint Nomor ' . $no_complaint . ' Dengan Alasan : "' . $request->alasan_tidak_terlibat . '"']);
    ...
    public function complaintTerlibat(Request $request){
        ...
        DB::table('divisi_complaint')->where('nomor_complaint', $no_complaint)->update(['timbangan' => 2, 'alasan_timbangan' => null]);
        ...
        date_default_timezone_set('Asia/Jakarta');
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $no_complaint, 'divisi' => 4, 'action' => 'Divisi Timbangan Merubah Status Menjadi Terlibat Pada Complaint Nomor ' . $no_complaint]);
        ModelComplaintCust::where('nomor_complaint', $no_complaint)->update(['updated_at' => date("Y-m-d H:i:s")]);
    }
}
