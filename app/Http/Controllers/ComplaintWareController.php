...
class ComplaintWareController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($request->hasFile('upload_file_ware')) {
            $file = $request->file('upload_file_ware');
            $nama_file = time()."_".$request->custid_ware."_".$request->nomor_complaint_ware."_".$file->getClientOriginalName();
            $tujuan_upload = 'data_file';
            $file->move($tujuan_upload, $nama_file);

            $data_komitmen = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_ware)->where('divisi', 5)->first();

            if($data_komitmen){
                $data_komitmen_get = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_ware)->where('divisi', 5)->get();
            ...
            $complaint=DB::table('complaint_warehouse')->insert(["nomor_complaint" => $request->nomor_complaint_ware, "evaluasi" => $request->evaluasi_ware, "solusi_internal" => $request->solusi_internal_ware, "solusi_customer" => $request->solusi_customer_ware, "lampiran" => $nama_file, "created_by" => Session::get('id_user_admin'), "tanggal_input" => date('Y-m-d')]);

            if($complaint){
                $arr = array('msg' => 'Data Successfully Stored', 'status' => true);
                DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_ware)->update(['baca' => 0, 'warehouse' => 2]);

                $stat = DB::table('status_complaint')->select(DB::raw("(if(produksi = 2, 2, 0) + if(logistik = 2, 2, 0) + if(sales = 2, 2, 0) + if(timbangan = 2, 2, 0) + if(warehouse = 2, 2, 0) + if(lab = 2, 2, 0) + if(lainnya = 2, 2, 0)) as hasil"))->where('nomor_complaint', $request->nomor_complaint_ware)->first();

                $div = DB::table('divisi_complaint')->select(DB::raw("(if(produksi = 2, 2, 0) + if(logistik = 2, 2, 0) + if(sales = 2, 2, 0) + if(timbangan = 2, 2, 0) + if(warehouse = 2, 2, 0) + if(lab = 2, 2, 0) + if(lainnya = 2, 2, 0)) as hasil"))->where('nomor_complaint', $request->nomor_complaint_ware)->first();

                if($stat->hasil == $div->hasil){
                    DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_ware)->update(['validasi' => 1]);
                }else{
                    DB::table('status_complaint')->where('nomor_complaint', $request->nomor_complaint_ware)->update(['validasi' => 0]);
                }
            }
        }else{
            $data_komitmen = DB::table('temp_list_action')->select('id_user', 'nomor_complaint', 'divisi', 'selesai_tanggal_komitmen', 'komitmen')->where('nomor_complaint', $request->nomor_complaint_ware)->where('divisi', 5)->first();
            ...
            $complaint=DB::table('complaint_warehouse')->insert(["nomor_complaint" => $request->nomor_complaint_ware, "evaluasi" => $request->evaluasi_ware, "solusi_internal" => $request->solusi_internal_ware, "solusi_customer" => $request->solusi_customer_ware, "created_by" => Session::get('id_user_admin'), "tanggal_input" => date('Y-m-d')]);
        ...
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $request->nomor_complaint_ware, 'divisi' => 5, 'action' => 'Divisi Warehouse Berhasil Memproses dan Memasukkan Data pada Complaint Nomor ' . $request->nomor_complaint_ware]);
        ModelComplaintCust::where('nomor_complaint', $request->nomor_complaint_ware)->update(['updated_at' => date("Y-m-d H:i:s")]);
    ...
    public function show($nomor_complaint){   
        ...
        $data  = DB::table('complaint_warehouse as com')->select('com.nomor_complaint', 'com.evaluasi', 'com.solusi_internal', 'com.solusi_customer', 'com.lampiran')->where('com.nomor_complaint', $val_nomor_complaint)->first();
     
        $komitmen = DB::table('list_of_action_complaint')->select('komitmen', 'selesai_tanggal_komitmen')->where('nomor_complaint', $val_nomor_complaint)->where('divisi', 5)->get();
     
        return Response()->json(["data_complaint" => $data, "data_komitmen" => $komitmen]);
    }

    public function showDataWarehouse($no_complaint){   
    ...
    public function complaintTdkTerlibat(Request $request){
        ...
        DB::table('divisi_complaint')->where('nomor_complaint', $no_complaint)->update(['warehouse' => 1, 'alasan_warehouse' => $request->alasan_tidak_terlibat]);
        ...
        date_default_timezone_set('Asia/Jakarta');
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $no_complaint, 'divisi' => 5, 'action' => 'Divisi Warehouse Merubah Status Menjadi Tidak Terlibat Pada Complaint Nomor ' . $no_complaint . ' Dengan Alasan : "' . $request->alasan_tidak_terlibat . '"']);
    ...
    public function complaintTerlibat(Request $request){
        ...
        DB::table('divisi_complaint')->where('nomor_complaint', $no_complaint)->update(['warehouse' => 2, 'alasan_warehouse' => null]);
        ...
        date_default_timezone_set('Asia/Jakarta');
        DB::table('logbook_complaint')->insert(['tanggal' => date("Y-m-d H:i:s"), 'nomor_complaint' => $no_complaint, 'divisi' => 5, 'action' => 'Divisi Warehouse Merubah Status Menjadi Terlibat Pada Complaint Nomor ' . $no_complaint]);
        ModelComplaintCust::where('nomor_complaint', $no_complaint)->update(['updated_at' => date("Y-m-d H:i:s")]);
    }
}
